...
app_uploadCL = Blueprint('uploadCL', __name__,
                       url_prefix='/uploadCL',
                       template_folder='templates/uploadyCL/',
...
def uploadCL_save(file_object):
    ...
    uploadCL_location = os.path.join(path, filename)
    # save file object to upload location
    file_object.save(uploadCL_location)
    return filename
...
def uploadCL():
    ...
    # load content page
    return render_template('uploadCL.html', user=user, files=files_uploaded)


# Notes create/add
@app_uploadCL.route('/upload/', methods=["POST"])
@login_required
def uploaders():
    try:
        ...
        filename = uploadCL_save(fo)

        # inserts location of object to feedback list
        files_uploaded.insert(0, url_for('uploadsCL_endpoint', name=filename))
    except:
        # errors handled, but specific errors are not messaged to user
        pass
    # reload content page
    return redirect(url_for('uploadCL.uploadCL'))